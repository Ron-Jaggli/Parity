...
| **Occlusion culling** | Switches occlusion culling on for any camera that has it off. | It only skips renderers the baked data proves cannot be seen from where you stand — it can remove work, never anything visible. With no baked data it does nothing. Cameras that already had it on are untouched. |
| **No redundant vsync** | Sets `vSyncCount = 0` while XR is running. | The headset's compositor already decides when a frame is presented; Unity's vsync is a second gate blocking on a desktop vblank you aren't looking at. Never applied outside VR. |
...
| `DisableOffscreenSkinnedMeshUpdates` | Large, with a lot of community content installed. | Clears `updateWhenOffscreen`, which forces per-frame skinning and bounds recomputation on the CPU. That flag is the standard fix for a mesh with wrong bounds — so if a mesh genuinely needs it, clearing it lets Unity cull the mesh while it is still on screen. The riskiest setting here: turn it on, look at your avatars and NPCs, turn it off if anything blinks out. |
| `DisableRealtimeReflectionProbes` | Large where a probe is refreshing every frame — six scene re-renders per frame on a mobile GPU. | If a probe is genuinely meant to update, this freezes the reflection, and a frozen reflection is visible. |
